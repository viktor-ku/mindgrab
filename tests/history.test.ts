...
test("undo restores creation, edits, reorder, movement, and deleted subtrees in order", () => {
  ...
  change(translateSubtree(nodes, "a", new Map(layoutMindMap(nodes).nodes.map((node) => [node.id, node])), { x: 80, y: -40 }));
  change(deleteNode(nodes, "a"));
  expect(nodes).toEqual([]);
  for (const state of states.reverse()) {
    const restored = history.undo()!;
    expect(restored).toEqual(state);
    expect(layoutMindMap(restored.nodes, restored.sizes, restored.anchor)).toEqual(
      layoutMindMap(state.nodes, state.sizes, state.anchor),
    );
  }
...
test("a completed gesture uses one entry and unchanged operations use none", () => {
  ...
  history.record(before, updateNode(before.nodes, "a", (node) => ({ ...node, text: "A" })));
  expect(history.undo()).toBeUndefined();
  const positions = new Map(layoutMindMap(before.nodes).nodes.map((node) => [node.id, node]));
  let dragged = before.nodes;
  for (let x = 1; x <= 150; x++) dragged = translateSubtree(before.nodes, "a", positions, { x, y: 30 });
...
test("only the latest 100 changes remain available", () => {
  const history = createHistory();
  for (let i = 0; i < 105; i++) {
    history.record(snapshot([{ id: "a", text: String(i) }]), [{ id: "a", text: String(i + 1) }]);
  }
  for (let i = 104; i >= 5; i--) expect(history.undo()?.nodes[0].text).toBe(String(i));
  expect(history.undo()).toBeUndefined();
});
